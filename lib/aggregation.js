...
function supportAggregate(Meanio) {
    ...
    Meanio.onModulesFoundAggregate = function () {
        for (var name in Meanio.modules) {
            var aggregator = new Aggregator({}, Meanio.modules[name], false, false);//Meanio.Singleton.config.clean.debug); //XXX:Aggregator的option部分需要考虑来源
