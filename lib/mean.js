...
Meanio.prototype.serve = function (options, callback) {
    ...
    var database = mongoose.connect(defaultConfig.db || '', defaultConfig.dbOptions || {}, function (err) {
        ...
        Meanio.Singleton.config = new Config(function (err, config) {
            ...
            Meanio.events.on('modulesFound', Meanio.onModulesFoundAggregate.bind(null, options.debug));

//      Meanio.Singleton.aggregate(null,'js', null,{},false);//实际上仅执行了： Meanio.events.on('modulesFound', Meanio.onModulesFoundAggregate.bind(null, ext, options));
...
/**
 * 对于menu，应该归属于module，
 * 每个menu也应该有自己的权限，相应的角色可以访问相应的菜单.
 * @constructor
 */
Meanio.prototype.Menus = function () {
    this.add = function (newmenus) {
        if (!Array.isArray(newmenus)) newmenus = [newmenus];
...
//    });
        ...
        newmenus.forEach(function (menu) {
            if (menu.name) {
                menu.icon = menu.icon || 'fa-th';
                menu.roles = menu.roles || ['anonymous'];
                if (menu.moduleName) {
                    allMenus[menu.moduleName] = allMenus[menu.moduleName] || [];
                    allMenus[menu.moduleName].push(menu);
                } else {
                    console.log('菜单不知来源于何处而被忽略！');
                }
            }
        });
    ...
    /**
     * @param options 提供moduleName和roles
     * @returns {Array}
     */
    this.get = function (options) {
        var allowed = [];
        if (!options.moduleName)return [];

        options.roles = options.roles || ['anonymous'];

        if(!allMenus[options.moduleName])allMenus[options.moduleName]=[];
        allMenus[options.moduleName].forEach(function (item) {
            var hasRole = false;
            options.roles.forEach(function (role) {
                if (role === 'admin' || item.roles.indexOf('anonymous') !== -1 || item.roles.indexOf(role) !== -1) hasRole = true;
            });

            if (hasRole) allowed.push(item);
        });
        return allowed;
//    var allowed = [];
//    options = options || {};
...
Module.prototype.addMenu = function (options) {
    options.moduleName = this.name;
    Meanio.Singleton.menus.add(options);
};
...
function findModules(callback) {
                ...
                fs.readFile(path.join(process.cwd(), source, file, 'package.json'), function (fileErr, data) {
                    if (data) {
                        try {
                            ...
                            //一般分：terminal，manager和缺省(default)三类，非terminal和manager的统统视为default用户
                            if (json.mean) {
                                Meanio.modules[file] = {
                                    title: json.title || json.name,
                                    version: json.version,
                                    usertype: json.usertype && ('terminal' === json.usertype || 'manager' === json.usertype || '$_techrich_$' === json.usertype) ? json.usertype : 'website',
                                    source: source,
                                    icon: json.icon,
                                    index_link: json.index || json.name
                                };
                            }
                        } catch (err) {
    ...
    Q.all([searchSource('node_modules'), searchSource('system'), searchSource('packages/terminal'), searchSource('packages/manager'), searchSource('packages/website')]).done(function () {
